...
- В учебном проекте корзина и заказы хранятся в памяти — данные теряются при перезапуске.
- Для продакшена используйте базу данных и хранение секретов через GitHub Secrets или переменные окружения.
- ID товаров — индекс в `shop.json` (начинается с 0).

**Дополнительные задания (по желанию):**

1. Перенесите хранение корзины в файл или простую БД (например SQLite).
2. Добавьте логирование в приложение.

Перед пушем полезно запускать локально: `ruff check .`, `pytest -v test_main.py` — тогда CI не будет падать на линтере и тестах.
